frappe.pages["abhilasha-clf-dashboard"].on_page_load = function (wrapper) {
	const page = frappe.ui.make_app_page({
		parent: wrapper,
		title: "Abhilasha CLF Dashboard",
		single_column: true,
	});
			...
			<div class="abh-shell">
				<header class="abh-header">
					<div>
						...
						<h1 data-i18n="title">Abhilasha CLF Dashboard</h1>
...
frappe.pages["abhilasha-clf-dashboard"].on_page_show = function () {
...
const abhilasha_dashboard = {
	...
	get_route_state() {
		...
		const pagePathIndex = pathParts.indexOf("abhilasha-clf-dashboard");
		const routeParts = route[0] === "abhilasha-clf-dashboard" ? route.slice(1) : route;
	...
	update_dashboard_route() {
		this.replace_route("/app/abhilasha-clf-dashboard");
	},
	...
	update_list_route() {
		...
		this.replace_route(`/app/abhilasha-clf-dashboard?${params.toString()}`);
	},
